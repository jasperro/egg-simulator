...
func _ready():
	...
	reset_level.connect(get_node("/root/World")._reset_level)
	reset_timer.connect(timercounter._reset)
	level_won.connect(win_popup._level_won)
...
func _last_hurrah():
	...
	# Kapot ei genereren
	splitegg = load("res://Assets/Models/Scenes/SplitEgg.tscn").instantiate()
	$"..".add_child(splitegg)
...
func process_input(_delta):
	...
	if Input.get_accelerometer():
		apply_central_force(camera.get_global_transform().basis.z * Vector3(0, 0, Input.get_accelerometer()[2]*40+200) + Vector3(Input.get_accelerometer()[0]*-30,0,0))

	if Input.is_action_pressed("movement_forward"):
		apply_central_force(-camera.get_global_transform().basis.z * SPEED_MODIFIER)
	if Input.is_action_pressed("movement_back"):
		apply_central_force(camera.get_global_transform().basis.z * SPEED_MODIFIER)
	if Input.is_action_pressed("movement_left"):
		apply_central_force(-camera.get_global_transform().basis.x * SPEED_MODIFIER)
...
func _input(event):
	if event is InputEventScreenDrag:
		...
		rotator_node.rotate_y(deg_to_rad(y_change_deg))
